      ...
      <div class="container">
        <div class="container-ficha">
          <div class="texto-areas">
            ...
            <p>Nombre: {{this.user.nombre_usuario}}</p>
            <p>Apellidos: {{this.user.apellidos_usuario}}</p>
            <p>Rol: {{this.user.rol_usuario}}</p>
            <button class="btnDel" v-on:click="borrarUser()">Eliminar Usuario</button>
          </div>
        </div>
      </div>
    </div>    
  ...
  methods: { 
    async borrarUser() {
      let result = await axios.post (
        `https://icalldibs.000webhostapp.com/?servicio=eliminar_usuario`, {
